...
class SkioPlanPickerComponent extends LitElement {
          ...
          <label class="skio-group-label" for="skio-one-time-${ this.key }">
            <div class="skio-group-topline">
              ...
              <div class="skio-price">
                <span skio-onetime-price>${ this.moneyFormatter.format(parseFloat(this.selectedVariant.priceV2.amount)) }</span>
              </div>
            </div>
          </label>
        </div>
  ...
  updated = (changed) => {
    ...
    if(changed.has('selectedVariant')) {
      ...
      if(this.selectedVariant.priceV2.currencyCode) {
        this.moneyFormatter = new Intl.NumberFormat(this.language, {
          style: 'currency',
          currency: this.selectedVariant.priceV2.currencyCode
        });
      }
    }
  ...
  discount(selling_plan) {
    ...
    const price = parseFloat(this.selectedVariant.priceV2.amount) * 100;
  ...
  price(selling_plan, formatted = true) {
    return formatted
      ? this.money((parseFloat(this.selectedVariant.priceV2.amount) * 100) - this.discount(selling_plan).amount)
      : (parseFloat(this.selectedVariant.priceV2.amount) * 100) - this.discount(selling_plan).amount
  }
...
export default createComponent(React, 'skio-plan-picker', SkioPlanPickerComponent)